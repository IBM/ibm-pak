...
The plugin is also provided in a container image `cp.icr.io/cpopen/cpfs/ibm-pak:TAG` where `TAG` should be replaced with the corresponding plugin version, for example `cp.icr.io/cpopen/cpfs/ibm-pak:v1.11.1` will have `v1.11.1` of the plugin.
...
```
id=$(docker create cp.icr.io/cpopen/cpfs/ibm-pak:v1.11.1 - )
...
```
id=$(podman create cp.icr.io/cpopen/cpfs/ibm-pak:v1.11.1 - )

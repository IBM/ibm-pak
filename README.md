...
This repository provides the IBM Catalog Management Plug-in for IBM Cloud Paks 1.0 (tech preview) via its github releases. The plugin streamlines the deployment of IBM Cloud Paks in a disconnected environment which was done earlier using [cloudctl] (https://github.com/IBM/cloud-pak-cli).
...
```
openssl x509 -inform pub -in ibm-pak-plugin.pem -noout -text
```
...
oc image mirror -f ~/.ibm-pak/data/mirror/$CASE_NAME/$CASE_VERSION/images-mapping.txt --filter-by-os=.* --insecure --skip-multiple-scopes --max-per-registry=1
```
...
oc image mirror -f ~/.ibm-pak/data/mirror/$CASE_NAME/$CASE_VERSION/images-mapping-to-filesystem.txt --filter-by-os=.* --insecure --skip-multiple-scopes --max-per-registry=1

# manually copy filesystem v2 directory contents to new machine

oc image mirror -f ~/.ibm-pak/data/mirror/$CASE_NAME/$CASE_VERSION/images-mapping-from-filesystem.txt --filter-by-os=.* --insecure --skip-multiple-scopes --max-per-registry=1
```
